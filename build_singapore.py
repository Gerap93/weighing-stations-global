#!/usr/bin/env python3
"""
Genera location/singapore/data.json a partir del CSV de Singapore.

Singapore usa el MISMO modelo que Séneca/Dubai (Dory): fórmulas, líneas, lab/robot,
ingredientes por fórmula. Reutiliza casi toda la lógica de build_data.py (clasificación,
scopes, proyectos, integridad). Solo cambia la LECTURA del CSV, porque el export de
Singapore tiene sus propias particularidades:
  · columnas en INGLÉS como Dubai (Formula code, Ingredient resp., Prod. end…)
  · encoding UTF-8; separador ';'; fecha dd/mm/yyyy con hora "3:36:54 pm" (parse_date
    ya toma solo la parte de fecha, así que am/pm no molesta)
  · SIN robot: "Ingredient resp." siempre relleno (SGPDORY1/SGPDORY2) → todo Lab
  · PESO EN MILIGRAMOS ("Ingr. weight [mg]"), no en gramos como Séneca/Dubai. Se
    convierte a GRAMOS (/1000) para que sea comparable en la vista global.
  · FORMATO NUMÉRICO MIXTO en el mismo archivo:
      - valores pequeños con coma decimal:      "1,5"       = 1.5
      - lotes grandes con coma de miles (US):   "1,200,000" = 1200000
    Regla robusta comprobada sobre los datos: 2+ comas → separador de miles (se quitan);
    exactamente 1 coma → decimal (coma→punto). No hay valores de 1 sola coma que sean
    miles redondos, así que la regla no es ambigua.

Uso:
    python build_singapore.py
Lee:  ./location/singapore/*.csv
Escribe: ./location/singapore/data.json
"""
...
SRC_DIR = os.path.join(BASE, "location", "singapore")
OUT = os.path.join(BASE, "location", "singapore", "data.json")

# Mapeo columna interna ← cabecera del CSV de Singapore (inglés). Nota: el peso viene
# en MILIGRAMOS (a diferencia de Dubai/Séneca, que vienen en gramos).
COLS = {
    'cod':       'Formula code',
    'nom':       'Formula name',
    'pesoIngr':  'Ingr. weight [mg]',
...
def num_sgp(s):
    """Número de Singapore → GRAMOS.

    Formato mixto en el mismo CSV:
      · coma decimal en valores pequeños:   "1,5"       → 1.5
      · coma de miles (US) en lotes grandes:"1,200,000" → 1200000
    Regla: 2+ comas ⇒ miles (se eliminan); 1 coma ⇒ decimal (coma→punto).
    El CSV está en mg; se divide entre 1000 para dejarlo en gramos y que sea
    comparable con Séneca/Dubai.
    """
    s = (s or '').strip()
    if not s:
        return 0.0
    if s.count(',') >= 2:
        s = s.replace(',', '')       # separadores de miles US
    else:
        s = s.replace(',', '.')      # coma decimal → punto
    try:
        return float(s) / 1000.0     # mg → g
    except ValueError:
        return 0.0


def cargar_filas():
    """Lee el/los CSV de Singapore y devuelve (rows, meta), misma forma que build_data."""
    rows = []
    archivos = []
